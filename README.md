...
| `azureSignIn.scope` | List\<String\> | ['https://management.core.windows.net//.default', 'offline_access', 'openid', 'profile'] | :heavy_check_mark: | A space-separated list of scopes that you want the user to consent to. For the /authorize leg of the request, this parameter can cover multiple resources. This value allows your app to get consent for multiple web APIs you want to call. Uses the **az cli** Scopes by default |
...
| `azureSignIn.signInUri` | String | https://login.microsoftonline.com/organizations/oauth2/v2.0/authorize?client_id=\$clientId&response_type=code&redirect_uri=http://localhost:\$port&scope=\$scopeString&response_mode=form_post | :heavy_multiplication_x: | Getter for the Microsoft Sign-In URL used to Sign In via Browser. Combines `azureSignIn.clientId`, `azureSignIn.port` and `azureSignIn.scope` |
...
| `token.status` | int | 0: Success, 1: Azure API error, 2: HttpServer error, 3: Sign In cancelled | Status of the Token authorization code flow result. |
